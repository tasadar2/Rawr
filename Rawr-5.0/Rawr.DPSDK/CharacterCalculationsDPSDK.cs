...
namespace Rawr.DPSDK
{
    public class CharacterCalculationsDPSDK : CharacterCalculationsBase
    {
        ...
        public void DPSBreakdown(Rotation rot)
        {
            ...
            damSub[(int)DKability.FrostFever] = rot.GetAbilityOfType(DKability.FrostFever).DPS * rot.CurRotationDuration;
            damSub[(int)DKability.BloodPlague] = rot.GetAbilityOfType(DKability.BloodPlague).DPS * rot.CurRotationDuration;
            threatSub[(int)DKability.FrostFever] = rot.GetAbilityOfType(DKability.FrostFever).TPS * rot.CurRotationDuration;
            threatSub[(int)DKability.BloodPlague] = rot.GetAbilityOfType(DKability.BloodPlague).TPS * rot.CurRotationDuration;
            // DPS like this works for everything except the diseases or pets.  
            // (anything that lasts longer than the rotation.)
            foreach (DKability b in EnumHelper.GetValues(typeof(DKability)))
            {
                dpsSub[(int)b] = fTotalDPS * (damSub[(int)b] / fTotalDam);
                tpsSub[(int)b] = rot.m_TPS * (threatSub[(int)b] / rot.TotalThreat);
            }
            dpsSub[(int)DKability.Army] = PetDPS[(int)DKability.Army];
            dpsSub[(int)DKability.BloodParasite] = PetDPS[(int)DKability.BloodParasite];
            dpsSub[(int)DKability.Gargoyle] = PetDPS[(int)DKability.Gargoyle];
            dpsSub[(int)DKability.Ghoul] = PetDPS[(int)DKability.Ghoul];
            dpsSub[(int)DKability.FrostFever] = rot.GetAbilityOfType(DKability.FrostFever).DPS;
            dpsSub[(int)DKability.BloodPlague] = rot.GetAbilityOfType(DKability.BloodPlague).DPS;
            tpsSub[(int)DKability.FrostFever] = rot.GetAbilityOfType(DKability.FrostFever).TPS;
            tpsSub[(int)DKability.BloodPlague] = rot.GetAbilityOfType(DKability.BloodPlague).TPS;
        }
