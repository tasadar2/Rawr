...
namespace Rawr.Enhance
{
    public class EnhSim
    {
        ...
        public EnhSim(Character character, CalculationOptionsEnhance calcOpts, BossOptions bossOpts)
        {
            ...
            // Stats from buffs are not exported, except for unleashed rage as it can not be "turned off".  Apply AP and SP from UR here
            ...
            float hasteBonus = StatConversion.GetPhysicalHasteFromRating(stats.HasteRating, CharacterClass.Shaman) * 100f;
            ...
            {
                if (character.MainHand.Id == 78472)
                    weaponType = "nokaled_heroic";
                else if (character.MainHand.Id == 77188)
                    weaponType = "nokaled";
                else if (character.MainHand.Id == 78481)
                    weaponType = "nokaled_rf";
                else if (character.MainHand.Type == ItemType.OneHandAxe || character.MainHand.Type == ItemType.TwoHandAxe)
                    weaponType = "axe";
                else if (character.MainHand.Type == ItemType.FistWeapon)
                    weaponType = "fist";
                else if (character.MainHand.Type == ItemType.OneHandMace || character.MainHand.Type == ItemType.TwoHandMace)
                    weaponType = "mace";
            }
            ...
            {
                if (character.OffHand.Id == 78472)
                    weaponType = "nokaled_heroic";
                else if (character.OffHand.Id == 77188)
                    weaponType = "nokaled";
                else if (character.OffHand.Id == 78481)
                    weaponType = "nokaled_rf";
                else if (character.OffHand.Type == ItemType.OneHandAxe)
                    weaponType = "axe";
                else if (character.OffHand.Type == ItemType.FistWeapon)
                    weaponType = "fist";
                else if (character.OffHand.Type == ItemType.OneHandMace)
                    weaponType = "mace";
            }
        ...
        private void addBuffs(Character character, StringBuilder sb)
        {
            ...
            if (_character.ActiveBuffsContains("Trueshot Aura") || _character.ActiveBuffsContains("Unleashed Rage") ||
                ...
                sb.AppendLine("attack_power_buff_multiplier    20.0/20.0");
            else
                sb.AppendLine("attack_power_buff_multiplier    0.0/20.0");
                ...
                foreach (KeyValuePair<string, int> kvp in setCounts)
                {
                    switch (kvp.Key)
                    {
                        ...
                        case "Volcanic Battlegear":
                            ...
                            break;
                        case "Spiritwalker's Battlegear":
                            if (kvp.Value >= 4)
                            {
                                bonusCount++;
                                sb.AppendLine("set_bonus" + bonusCount + "                      t13_battlegear_4");
                            }
                            else if (kvp.Value >= 2)
                            {
                                bonusCount++;
                                sb.AppendLine("set_bonus" + bonusCount + "                      t13_battlegear_2");
                            }
                            break;
                    }
                }
