...
namespace Rawr.ProtWarr
{
    ...
    public class DefendTable : CombatTable
    {
        ...
        protected override void Calculate()
        {
            ...
            const float CTCap = 1.024f; // Combat Table Cap (CTC) is set to 102.4%

            // Miss
            Miss = Math.Min(CTCap - tableSize, Lookup.AvoidanceChance(Player, HitResult.Miss));
            tableSize += Miss;
            // Dodge
            Dodge = Math.Min(CTCap - tableSize, Lookup.AvoidanceChance(Player, HitResult.Dodge));
            tableSize += Dodge;
            // Parry
            Parry = Math.Min(CTCap - tableSize, Lookup.AvoidanceChance(Player, HitResult.Parry));
            tableSize += Parry;
            // Block
            if (Player.Character.OffHand != null && Player.Character.OffHand.Type == ItemType.Shield)
            {
                Block               = Math.Min(CTCap - tableSize, Lookup.AvoidanceChance(Player, HitResult.Block));
                BaseBlock           = Block;
                BuffedBlock         = Block;

                CriticalBlock       = Block * Math.Min(CTCap, Lookup.AvoidanceChance(Player, HitResult.CritBlock));
                ...
                if (Player.CalcOpts.UseShieldBlock)
                {
                    ...
                    BuffedBlock         = Math.Min(CTCap - tableSize, Lookup.AvoidanceChance(Player, HitResult.Block) + 0.25f);
                    BuffedCriticalBlock = BuffedBlock * Math.Min(1.0f, Lookup.AvoidanceChance(Player, HitResult.CritBlock) + (0.25f - (BuffedBlock - Block)));

                    Block               = (Block * (CTCap - shieldBlockUptime)) + (BuffedBlock * shieldBlockUptime);
            ...
            // Critical Hit
            Critical = Math.Min(CTCap - tableSize, Lookup.TargetCritChance(Player));
            tableSize += Critical;
            // Normal Hit
