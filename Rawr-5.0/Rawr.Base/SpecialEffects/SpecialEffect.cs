...
//        public static int numSpecEffectsCreatedEver = 0;
        ...
        // these should be used to modify elemental damage values by character spell/attack power
        ...
        public SpecialEffect(Trigger trigger, Stats stats, float duration, float cooldown, float chance, float spscaling, int maxStack, bool limitedtoexecutephase = false)
        {
            Chance = chance;
            MaxStack = maxStack;
            Trigger = trigger;
            ...
            LimitedToExecutePhase = limitedtoexecutephase;
            //Interlocked.Increment(ref numSpecEffectsCreatedEver);
            SpellPowerScaling = spscaling;
        }

        public SpecialEffect(Trigger trigger, Stats stats, float duration, float cooldown, float chance, int maxStack, float apscaling, bool limitedtoexecutephase = false )
        {
            Chance = chance;
            MaxStack = maxStack;
            Trigger = trigger;
        ...
        private string SPScalingString {
            get
            {
                return ", modified by " + (SpellPowerScaling * 100) + "% Spell Power";
            }
        }

        private string APScalingString
        {
            get
            {
                return ", modified by " + (AttackPowerScaling * 100) + "% Attack Power";
            }
        }
        ...
        public override string ToString()
        {
            ...
            if (SpellPowerScaling > 0)
            {
                s.Append(SPScalingString);
            }
            if (AttackPowerScaling > 0)
            {
                s.Append(APScalingString);
            }
            s.Append(")");
            return s.ToString();
        }
