...
namespace Rawr.DPSWarr {
    public class ArmsRotation : Rotation {
        ...
        private static readonly SpecialEffect[][] _SE_ChargeUse = {
            new SpecialEffect[] { new SpecialEffect(Trigger.Use, null, 10, (15f + 0 * 0f - 1f * 0f)), new SpecialEffect(Trigger.Use, null, 10, (15f + 0 * 0f - 1f * 1f)) },
            new SpecialEffect[] { new SpecialEffect(Trigger.Use, null, 10, (15f + 1 * 0f - 1f * 0f)), new SpecialEffect(Trigger.Use, null, 10, (15f + 1 * 0f - 1f * 1f)) },
        };
        #endregion
        ...
        public float SettleAll(float totalPercTimeLost, float rageUsedByMaintenance, float availRageO20, out float percFailRageO20)
        {
            ...
            */
            ...
            AbilityWrapper CH = GetWrapper<Charge>();
            AbilityWrapper SL = GetWrapper<Slam>();
            AbilityWrapper SoO = GetWrapper<StrikesOfOpportunity>();
            ...
            AbilityWrapper OP = GetWrapper<Overpower>();
            AbilityWrapper VR = GetWrapper<VictoryRush>();
            AbilityWrapper BLS = GetWrapper<Bladestorm>();
            AbilityWrapper RD = GetWrapper<Rend>();
            AbilityWrapper TH = GetWrapper<ThunderClap>();
            AbilityWrapper TB = GetWrapper<TasteForBlood>();
            AbilityWrapper IR = GetWrapper<InnerRage>();
            AbilityWrapper DC = GetWrapper<DeadlyCalm>();
        ...
        public void MakeRotationAndDoDPS(bool setCalcs, float percTimeUnder20) {
            ...
            if (_needDisplayCalcs) {
                ...
                AbilityWrapper aw = GetWrapper<DeadlyCalm>();
                GCDUsage = aw.AllNumActivates > 0 ? GCDUsage.Insert(GCDUsage.IndexOf("Abilities") - 2,
                    ...
                    : GCDUsage;
                AbilityWrapper aw2 = GetWrapper<InnerRage>();
        ...
        public override void MakeRotationandDoDPS(bool setCalcs, bool needsDisplayCalculations) {
            try {
                ...
                new Base.ErrorBox()
                {
                    Title = "Error in creating Arms Rotation Detailsr",
