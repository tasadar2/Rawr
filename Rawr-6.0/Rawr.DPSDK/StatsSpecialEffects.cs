...
//        public Stats stats;
        ...
        public StatsSpecialEffects(DKCombatTable t, Rotation rot, BossOptions bo)
        {
            ...
            switch (m_Rot.curRotationType)
            {
                ...
                {
                    fSpellHitChance = m_Rot.GetAbilityOfType(DKability.Outbreak).HitChance;
                    fSpellCritChance = m_Rot.GetAbilityOfType(DKability.Outbreak).CritChance;
                    break;
                }
            }
            ...
            triggerChances.Add(Trigger.DamageTakenPutsMeBelow50PercHealth, 0.50f);
            triggerIntervals.Add(Trigger.DamageTakenPutsMeBelow50PercHealth, m_bo.DynamicCompiler_Attacks.AttackSpeed);
            ...
            #region DK Specials
            if (m_Rot.HasTrigger(Trigger.RunicEmpowerment))
            {
                triggerIntervals.Add(Trigger.RunicEmpowerment, m_Rot.CurRotationDuration / (m_Rot.CountTrigger(Trigger.RunicEmpowerment)));
                triggerChances.Add(Trigger.RunicEmpowerment, 1);
            }
            if (m_Rot.HasTrigger(Trigger.RunicCorruption))
            {
                triggerIntervals.Add(Trigger.RunicCorruption, m_Rot.CurRotationDuration / (m_Rot.CountTrigger(Trigger.RunicCorruption)));
                triggerChances.Add(Trigger.RunicCorruption, 1);
            }
