...
namespace Rawr.Windwalker {
    [Rawr.Calculations.RawrModelInfo("Windwalker", "spell_monk_windwalker_spec", CharacterClass.Monk)]
    public class CalculationsWindwalker : CalculationsBase
    {
            ...
            get
            {
                ...
                for (int tier = 0; tier < 4; ++tier)
                {
                    foreach (int meta in metas)
                        retval.AddRange(WindwalkerGemmingTemplateBlock(tier, meta));
                }

                foreach (int meta in metas)
                    retval.AddRange(WindwalkerCogwheelTemplateBlock(meta));

                return retval;
            }
        }

        private List<GemmingTemplate> WindwalkerGemmingTemplateBlock(int tier, int meta)
        {
            List<GemmingTemplate> retval = new List<GemmingTemplate>();
            retval.AddRange(new GemmingTemplate[] {
                CreateWindwalkerGemmingTemplate(tier, tierNames, brilliant, brilliant, brilliant, brilliant, meta), // Straight Intellect
                CreateWindwalkerGemmingTemplate(tier, tierNames, brilliant, reckless, brilliant, brilliant, meta), // Int/Haste/Int
                CreateWindwalkerGemmingTemplate(tier, tierNames, brilliant, potent, brilliant, brilliant, meta), // Int/Crit/Int
                CreateWindwalkerGemmingTemplate(tier, tierNames, brilliant, artful, brilliant, brilliant, meta), // Int/Mastery/Int
                CreateWindwalkerGemmingTemplate(tier, tierNames, brilliant, reckless, purified, brilliant, meta), // Int/Haste/Spirit
                CreateWindwalkerGemmingTemplate(tier, tierNames, brilliant, potent, purified, brilliant, meta), // Int/Crit/Spirit
                CreateWindwalkerGemmingTemplate(tier, tierNames, brilliant, artful, purified, brilliant, meta), // Int/Mastery/Spirit
            });
            return retval;
        }

        const int DEFAULT_GEMMING_TIER = 1;
        private GemmingTemplate CreateWindwalkerGemmingTemplate(int tier, string[] tierNames, int[] red, int[] yellow, int[] blue, int[] prismatic, int meta)
        {
        ...
        private List<GemmingTemplate> WindwalkerCogwheelTemplateBlock(int meta)
        {
            List<GemmingTemplate> retval = new List<GemmingTemplate>();
            retval.AddRange(new GemmingTemplate[] {
                    // Engineering cogwheel templates (meta and 2 cogs each, no repeats)
                    CreateWindwalkerCogwheelTemplate(meta, cog_fractured, cog_quick),
                    CreateWindwalkerCogwheelTemplate(meta, cog_fractured, cog_smooth),
                    CreateWindwalkerCogwheelTemplate(meta, cog_fractured, cog_sparkling),
                    CreateWindwalkerCogwheelTemplate(meta, cog_quick, cog_smooth),
                    CreateWindwalkerCogwheelTemplate(meta, cog_quick, cog_sparkling),
                    CreateWindwalkerCogwheelTemplate(meta, cog_smooth, cog_sparkling),
                });
            return retval;
        }

        private GemmingTemplate CreateWindwalkerCogwheelTemplate(int meta, int cogwheel1, int cogwheel2)
        {
            return new GemmingTemplate
            {
                Model = "Windwalker",
        ...
        public override ICalculationOptionsPanel CalculationOptionsPanel { get { if (calculationOptionsPanel == null) { calculationOptionsPanel = new CalculationOptionsPanelWindwalker(); } return calculationOptionsPanel; } }
        private ICalculationOptionsPanel calculationOptionsPanel = null;
        public override ComparisonCalculationBase CreateNewComparisonCalculation() { return new ComparisonCalculationWindwalker(); }
        public override CharacterCalculationsBase CreateNewCharacterCalculations() { return new CharacterCalculationsWindwalker(); }

        public override ICalculationOptionBase DeserializeDataObject(string xml)
        {
            System.Xml.Serialization.XmlSerializer serializer =
                new System.Xml.Serialization.XmlSerializer(typeof(CalculationOptionsWindwalker));
            System.IO.StringReader reader = new System.IO.StringReader(xml);
            CalculationOptionsWindwalker calcOpts = serializer.Deserialize(reader) as CalculationOptionsWindwalker;
            return calcOpts;
        }
        ...
        /// <summary>
        /// List of itemtypes that are relevant for Windwalker
        /// </summary>
        ...
        {
            CalculationOptionsWindwalker opts = character.CalculationOptions as CalculationOptionsWindwalker;
            if (opts == null)
                opts = new CalculationOptionsWindwalker();
        ...
        public override CharacterCalculationsBase GetCharacterCalculations(Character character, Item additionalItem, bool referenceCalculation, bool significantChange, bool needsDisplayCalculations)
        {
            ...
            CharacterCalculationsWindwalker calc = new CharacterCalculationsWindwalker();
            if (character == null)
                return calc;
            return calc;
        }
