...
//        private float _cachedHaste;
        ...
        private uint SingleGCD 
        {
            get 
            { 
                return (m_CT.m_Opts.presence == Presence.Unholy) ? MIN_GCD_MS_UH: MIN_GCD_MS;
            }
        }
        ...
        private float _meleeSpecials;
        public float m_MeleeSpecials
        {
            get { return _meleeSpecials; }
            set { _meleeSpecials = value; }
        }
        private float _spellSpecials;
        public float m_SpellSpecials
        {
        ...
        public void PRE_BloodDiseased()
        {
            ...
            {
                // Simple Outbreak, DSx4, HSx4, RS w/ RP (x3ish).
                ml_Rot.Add(OutB);
                ml_Rot.Add(FF);
...
//                ml_Rot.Add(BP); // Scarlet Fever is tied to this for 4.0.6
        ...
        public void PRE_Frost()
        {
            ...
            float RSCount = 0;
            // Burn what we can.
            for (RSCount = Math.Abs((float)m_RunicPower / (float)FS.RunicPower); RSCount >= 1; )
            {
                ml_Rot.Add(FS);
            ...
            if (m_CT.m_CState.m_Talents.Rime > 0)
            {
                ...
                if (fRimeMod > 0f && fRimeMod < 1f)
                {
                    ...
                    ability.AbilityCost[(int)DKCostTypes.Death] = 0;
                    ability.AbilityCost[(int)DKCostTypes.RunicPower] += 10; // The initial 10 RP are not gained, but Chill of the Grave still has value
                    ability.AbilityCost[(int)DKCostTypes.RunicPower] = (int)((float)ability.AbilityCost[(int)DKCostTypes.RunicPower] * fRimeMod);
            ...
            // Check the duration at this point... 
            // There is probably more time to fill the diseases.
            float DiseaseTimeLeftInGCDs = (BP.uDuration - m_RotationDuration) / SingleGCD;
            for (; DiseaseTimeLeftInGCDs >= 1; )
            {
                ml_Rot.Add(OB);
                m_RunicPower += OB.RunicPower;
                DiseaseTimeLeftInGCDs--;

                for (RSCount = Math.Abs((float)m_RunicPower / (float)FS.RunicPower); (RSCount >= 1 && DiseaseTimeLeftInGCDs >= 1); )
                {
                    ml_Rot.Add(FS);
                    m_RunicPower += FS.RunicPower;
                    RSCount--;
            ...
            #endregion
            BuildCosts();
            // Now that we've built the rotation, what do happens if we're GCD capped trying to do it all.  
            // What do we pull out?  
            // Rime procs or FSs?
            // Certainly whatever is least effective.
        ...
        /// <summary>
        /// Diseases > OB UU > HB > FS.
        ///
        /// This is not quite as detailed as the priority outlined on EJ, but
        /// I believe most of their complexity comes from trying to avoid
        /// capping RP.  We'll just assume RP never caps as a (hopefully)
        /// reasonable simplification.
        ///
        /// We also assume nothing ever misses.  I believe miss rate will still
        /// be subtracted from dps - but this code pretends it still
        /// costs/creates runes, diseases and RP as if it hit.  That certainly
        /// makes this less accurate, but hopefully not enough to change gearing
        /// choices.
        /// </summary>
        public void PRE_MasterFrost()
        {
            m_szRotationName = "Masterfrost Rotation - BETA";
            ResetRotation();
            ...
            // We also may end up adding a partial OB and a partial FS
            AbilityDK_Obliterate OBp = new AbilityDK_Obliterate(m_CT.m_CState);
            AbilityDK_FrostStrike FSp = new AbilityDK_FrostStrike(m_CT.m_CState);
            OBp.bPartial = true;
            FSp.bPartial = true;

            float duration = 0;
            float gcdAvailable = 0;
            float rpAvailable = 0;
            /// This is Unholy Available w/o RE... but the point is not to RE Unholy.
            float unholyAvailable = 0;
            float frostAvailable = 0;
            ...
            ml_Rot.Add(BP);
            --gcdAvailable;
            duration = BP.uDuration;
            // pretend we only have 1 unholy rune
            unholyAvailable = duration / m_SingleRuneCD;
            // include death runes with the frost count, since they are all used
            // for Howling Blast
            frostAvailable = unholyAvailable * 4;
            while (duration < 60 * 1000) {
                // our rotation will last the time between outbreak applications
                ml_Rot.Add(PS);
                --gcdAvailable;
                --unholyAvailable;
                rpAvailable -= PS.AbilityCost[(int)DKCostTypes.RunicPower];
                if (!m_CT.m_CState.m_Talents.GlyphofHowlingBlast)
                {
                    ml_Rot.Add(IT);
                    --gcdAvailable;
                    --frostAvailable;
                    rpAvailable -= IT.AbilityCost[(int)DKCostTypes.RunicPower];
                }
                // Add duration means more runes.
                duration += BP.uDuration;
                unholyAvailable += BP.uDuration / m_SingleRuneCD;
                frostAvailable += BP.uDuration * 4 / m_SingleRuneCD;
            }
            #endregion

            gcdAvailable += duration / SingleGCD;
            rpAvailable += duration / (5 * 1000) * m_CT.m_CState.m_Stats.RPp5;

            #region OB UU (Obliterate when both unholy runes are available)
            float added;
            for (added = 0; added + 1 <= unholyAvailable; ++added)
            {
                ml_Rot.Add(OB);
            }
#if false
		    if (added < unholyAvailable)
            {
                OBp.fPartialValue = unholyAvailable - added;
                ml_Rot.Add(OBp);
                added = unholyAvailable;
            }
  
#endif            
            gcdAvailable -= added;
            unholyAvailable -= added;
            frostAvailable -= added;
            rpAvailable -= added * OB.AbilityCost[(int)DKCostTypes.RunicPower];
            #endregion
            ...
            if (m_CT.m_CState.m_Talents.Rime > 0)
            {
                ...
                AbilityDK_Base ability = CreateRimeAbility(HB, IT);
                for (added = 0; added + 1 <= procs; ++added)
                {
                    ml_Rot.Add(ability);
                    --gcdAvailable;
                }
                ...
                if (added < procs)
                {
                    ability = CreateRimeAbility(HB, IT);
                    ability.bPartial = true;
                    ability.fPartialValue = procs - added;
                    ml_Rot.Add(ability);
                    gcdAvailable -= procs - added;
                }
#endif
                rpAvailable -= procs * ability.AbilityCost[(int)DKCostTypes.RunicPower];
            }
            #endregion

            #region non-RE HB
            for (added = 0; added + 1 <= frostAvailable; ++added)
            {
                ml_Rot.Add(HB);
                --gcdAvailable;
                --frostAvailable;
...
#if false
            if (added < frostAvailable)
            {
                AbilityDK_HowlingBlast HBp = new AbilityDK_HowlingBlast(m_CT.m_CState);
                HBp.bPartial = true;
                HBp.fPartialValue = frostAvailable - added;
                ml_Rot.Add(HBp);
                added = frostAvailable;
                gcdAvailable -= HBp.fPartialValue;
            }
#endif
            rpAvailable -= added * HB.AbilityCost[(int)DKCostTypes.RunicPower];
            #endregion

            #region FS & HB from RE
            while (gcdAvailable >= 1) 
            {
#if false
                if (frostAvailable >= 1)
                {
                    ...
                    --gcdAvailable;
                    --frostAvailable;
                    rpAvailable -= HB.AbilityCost[(int)DKCostTypes.RunicPower];
                }
                else  
#endif
                    
                    if (rpAvailable >= FS.AbilityCost[(int)DKCostTypes.RunicPower])
                {
                    ml_Rot.Add(FS);
                    --gcdAvailable;
                    frostAvailable += .45f;
                    rpAvailable -= FS.AbilityCost[(int)DKCostTypes.RunicPower];
                }
                else
                {
                    break;
...
#if false
            if (frostAvailable > 0 && gcdAvailable > 0)
            {
                float fraction = Math.Min(frostAvailable, gcdAvailable);
                AbilityDK_HowlingBlast HBp = new AbilityDK_HowlingBlast(m_CT.m_CState);
                HBp.bPartial = true;
                HBp.fPartialValue = fraction;
                ml_Rot.Add(HBp);
                gcdAvailable -= fraction;
                rpAvailable -= fraction * HB.AbilityCost[(int)DKCostTypes.RunicPower];
            } 
#endif
            if (rpAvailable > 0 && gcdAvailable > 0) {
                float fraction = Math.Min(
                    FS.AbilityCost[(int)DKCostTypes.RunicPower] / rpAvailable,
                    gcdAvailable);
                // It is possible the partial HB above gave us more than enough
                // RP for a full FS.  Let's pretend it didn't.
                fraction = Math.Min(fraction, 1);
                FSp.fPartialValue = fraction;
                ml_Rot.Add(FSp);
            }
            // If I was smarter I would figure out how to use up any remaining
            // resources without a loop doing increasingly-partial HB & FS. I'll
            // just stop here & call it good.
            #endregion
            ...
            if (m_CT.m_CState.m_Talents.KillingMachine > 0)
            {
                ...
                float fPctOBFSCrit = (fPPRot * m_CT.m_Opts.KMConsumeRate) / (OBFSCount / fNormalizer);
                OB.SetKMCritChance(fPctOBFSCrit);
                OBp.SetKMCritChance(fPctOBFSCrit);
                FS.SetKMCritChance(fPctOBFSCrit);
                FSp.SetKMCritChance(fPctOBFSCrit);
            }
            #endregion
        }

        private AbilityDK_Base CreateRimeAbility(
            AbilityDK_HowlingBlast HB, AbilityDK_IcyTouch IT)
        {
            AbilityDK_Base ability;
            // we want 1 full use, and then any sub values.
            if (HB.DPS > IT.DPS)
        ...
        public float PartialCount(DKability t)
        {
            float count = 0;
            if (null != ml_Rot)
            {
...
//            AbilityDK_PlagueStrike PS = new AbilityDK_PlagueStrike(m_CT.m_CState);
            ...
            for (int count = (int)(Dark.Cooldown / subrotDuration); count > 0; count--)
            {
                ...
                // Fill the disease.
                diseaseGCDs = Outbreak.Cooldown / SingleGCD; 
        ...
        public void PRE_Unholy()
        {
            ...
            if (m_CT.m_CState.m_Talents.SuddenDoom > 0)
            {
                ...
                if (fSDMod > 0 && fSDMod < 1)
                {
                    // we want 1 full use, and then any sub values.
                    ability = new AbilityDK_DeathCoil(m_CT.m_CState);
                    ability.szName = "DC (SD_Partial)";
                    ability.bPartial = true;
                    ability.fPartialValue = fSDMod;
                    // These are free DCs.
                    ability.AbilityCost[(int)DKCostTypes.Blood] = 0;
                    ability.AbilityCost[(int)DKCostTypes.Frost] = 0;
                    ability.AbilityCost[(int)DKCostTypes.UnHoly] = 0;
        ...
        public void BuildCosts()
        {
            ...
            if (null != ml_Rot)
            {
                foreach (AbilityDK_Base ability in ml_Rot)
                {
                    ...
                    {
                        if (ability.bPartial)
                            m_MeleeSpecials += ability.fPartialValue;
                        else
                            m_MeleeSpecials++;
                    }
                    ...
                    {
                        if (ability.bPartial)
                            m_SpellSpecials += ability.fPartialValue;
                        else
                            m_SpellSpecials++;
                    }
                    else
                    ...
                    if (ability.AbilityIndex == (int)DKability.BloodPlague
                        || ability.AbilityIndex == (int)DKability.FrostFever)
                    {
                        TotalDamage += ability.DPS * CurRotationDuration;
                        TotalThreat += ability.TPS * CurRotationDuration;
                    }
                    else
                    {
...
//            int DRCD = (m_DeathRunes - (int)BT_DeathRunes) * m_SingleRuneCD; // Blood Tap runes are free.
        ...
        public string ReportRotation()
        {
            ...
            string szFormat = "{0,-15}|{1,7:0}|{2,7:0.0}|{3,5:0.00}|{4,5:0.00}\n";
            //string szFormat = "{0,-15}|{3,4:0.00}|{4,4:0.00}|{1,7:0}|{2,7:0.0}\n";

            szReport += string.Format(szFormat, "Name", "Dam", "DPS", "Crit", "Hit");
            if (null != ml_Rot)
            {
                foreach (AbilityDK_Base ability in ml_Rot)
                {
                    szReport += string.Format(szFormat, ability.szName, ability.GetTotalDamage(), ability.GetDPS(), ability.CritChance, ability.HitChance);
                }
            }
            szReport += string.Format("Duration(sec): {0,6:0.0}\n", CurRotationDuration);
                    ...
                    switch (t)
                    {
                        ...
                        case Trigger.ScourgeStrikeHit:
                            if (ab.AbilityIndex == (int)DKability.ScourgeStrike)
                                iCount++;
                            break;
                        case Trigger.RunicCorruption:
                            if (ab.AbilityIndex == (int)DKability.DeathCoil)
                                iCount++;
                            break;
                    }
                }
                if (t == Trigger.RunicEmpowerment)
                    iCount = (int)Math.Round(m_FreeRunesFromRE);
            }
            return iCount;
        }
                ...
                foreach (AbilityDK_Base ab in ml_Rot)
                {
                    switch (t)
                    {
                        ...
                        case Trigger.ScourgeStrikeHit:
                            ...
                            break;
                        case Trigger.RunicCorruption:
                            bHasTrigger = (ab.AbilityIndex == (int)DKability.DeathCoil);
                            break;
                    }
                }
                if (t == Trigger.RunicEmpowerment)
                    bHasTrigger = (m_FreeRunesFromRE > 0);
            }
            return bHasTrigger;
        }
