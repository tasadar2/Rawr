...
namespace Rawr
{
    ...
    public partial class SpecialEffect
    {
        ...
        private class Parameters
        {
            ...
            private bool IsStationary(SpecialEffect effect)
            {
                if (effect.MaxStack > 1) return true;
                if (effect.Cooldown <= effect.Duration) return true;
                return false;
            }
                ...
                for (int i = 0; i < effects.Length; i++)
                {
                    if (!IsStationary(effects[i]))
                    {
                        ...
                        else
                        {
                            ...
                            if (discretizationCorrection && c[j] > 0)
                            {
                                c[j] += 0.5f;
                            }
                ...
                for (int i = 0; i < effects.Length; i++)
                {
                    if (IsStationary(effects[i]))
                    {
                        ...
                        if (effects[i].MaxStack > 1)
                        {
                            ...
                            v[j] *= effects[i].MaxStack;
                            uptime[j] = effects[i].GetAverageStackSize(triggerInterval[i], triggerChance[i], attackSpeed, fightDuration, r) / effects[i].MaxStack;
                        }
                        else
                        {
        ...
        public static WeightedStat[] GetAverageCombinedUptimeCombinations(SpecialEffect[] effects, float[] triggerInterval, float[] triggerChance, float[] offset, float[] scale, float attackSpeed, float fightDuration, float[] value, float[] resets = null)
        {
            ...
            {
                if (p.effects[0].MaxStack > 1)
                {
                    p.partialIntegral[0, 1] = fightDuration * p.uptime[0];
                }
                else
                {
