        ...
        public void AutoSetUniqueId(Item item)
        {
            if (item.Unique && (item.Slot == ItemSlot.Trinket || item.Slot == ItemSlot.Finger || item.Slot == ItemSlot.OneHand)) // all items that have UniqueId rules are marked as Unique
            {
                ...
                lock (Items)
                {
                    ...
                    foreach (Item i in list)
                    {
                        ...
                        if (i == validItem)
                        {
                            ...
                            else if (i.ItemLevel == 390) { item390 = i; }
                ...
                if ((object)item384 != null && (object)item397 != null && (object)item410 != null)
                {
                    ...
                    item397.UniqueId = new List<int>() { item384.Id, item397.Id, item410.Id };
                    item410.UniqueId = new List<int>() { item384.Id, item397.Id, item410.Id };
                }

                // LFR/normal/heroic Deathwing weapons with same name
                if ((object)item390 != null && (object)item403 != null && (object)item416 != null)
                {
