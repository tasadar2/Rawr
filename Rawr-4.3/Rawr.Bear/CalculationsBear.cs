...
the Threat Scale defined on the Options tab.",
        ...
        public override CharacterCalculationsBase GetCharacterCalculations(Character character, Item additionalItem, bool referenceCalculation, bool significantChange, bool needsDisplayCalculations)
        {
            ...
            // Special Attack + Auto Attack + Maul
            // Hard Coding 59.22737 is the average number Mangle attacks done in a 300 second time frame
            // TODO: This need changing once a better rotation module is implemented
            float manglecount = 59.22737f;
            float autoSpecialAttacksPerSecond = (stats.Tier_13_2_piece ? ((200f - manglecount) / 300f) : (1f / 1.5f)) + 1f / playerAttackSpeed + 1f / 3f;
            ...
            averageSDAttackCritChance += (stats.Tier_13_2_piece ? (playerChanceToCrit * manglecount / 300f): 0);
        ...
        private void CalculateThreat(StatsBear stats, int targetLevel, CharacterCalculationsBear calculatedStats, Character character)
        {
            ...
            float magicDPS = (stats.ShadowDamage + stats.ArcaneDamage + stats.NatureDamage + stats.FireDamage + stats.FrostDamage);
            magicDPS = (magicDPS * (1f - chanceCrit)) + (magicDPS * chanceCrit * critMultiplier);
            stats.PhysicalDamageProc *= stats.AttackPower;
            float physicalDPS = ((stats.PhysicalDamage + stats.PhysicalDamageProc) * (1f - chanceCrit)) + ((stats.PhysicalDamage + stats.PhysicalDamageProc) * (chanceCrit) * critMultiplier);
            float abomDPS = (stats.MoteOfAnger * abilities.MeleeStats.DamageAverage);
            float bonusdamage = magicDPS + physicalDPS + abomDPS;
        ...
        private static void AccumulateProcs(Character character, StatsBear statsTotal)
        {
            ...
            StatsBear statsProcs = new StatsBear();
            //float uptime;
            foreach (SpecialEffect effect in statsTotal.SpecialEffects())
            {
                ...
                if (effect.AttackPowerScaling > 0)
                    statsProcs.PhysicalDamageProc = effect.AttackPowerScaling;
            }
